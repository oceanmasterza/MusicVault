...
:class:`~musicvault.workers.io.scanner_worker.ScannerWorker` (size/mtime)
and :class:`~musicvault.workers.cpu.hash_worker.HashWorker` (content hash).
...
from musicvault.plugins.builtin.chromaprint import generate_chromaprint
...
def compute_fingerprint(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    :class:`~musicvault.workers.cpu.hash_worker.HashWorker`, which
