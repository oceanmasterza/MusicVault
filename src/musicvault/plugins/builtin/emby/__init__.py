"""Emby media-server plugin — library refresh via Emby REST API."""
...
class EmbyPlugin:
    """Triggers an Emby library scan using an API key (``token``).

    Same Emby-family endpoints as Jellyfin (``X-Emby-Token``,
    ``Library/Refresh``).
    """
