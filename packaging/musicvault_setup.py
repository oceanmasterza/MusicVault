"""MusicVault offline installer / uninstaller (Windows).

Built into ``MusicVault-Setup.exe`` by ``packaging/build_setup_exe.py``.
...
APP_NAME = "MusicVault"
APP_VERSION = "1.0.0"
PUBLISHER = "MusicVault"
PAYLOAD = "MusicVaultApp.zip"
...
def _log_path() -> Path:
    ...
    return Path(temp) / "MusicVault-Setup.log"
...
def _auto() -> bool:
    if any(a in {"--yes", "-y", "/S", "/s", "/silent"} for a in sys.argv[1:]):
        return True
    return os.environ.get("MUSICVAULT_SETUP_AUTO", "").strip().lower() in {"1", "true", "yes"}
...
def _open_log() -> Path:
    ...
    path.write_text(
        f"MusicVault Setup log ({mode}) — {datetime.now().isoformat(timespec='seconds')}\n"
        f"argv={sys.argv!r}\n"
...
def _write_arp(install_dir: Path, uninstall_exe: Path) -> None:
    ...
    display_icon = str(install_dir / "MusicVault.exe")
...
def _stop_running_app() -> None:
    """Best-effort stop of running MusicVault.exe before file delete."""
    if sys.platform != "win32":
        return
    result = subprocess.run(
        ["taskkill", "/IM", "MusicVault.exe", "/F"],
        check=False,
        capture_output=True,
        text=True,
    )
    if result.returncode == 0:
        log("Stopped running MusicVault.exe")
    else:
        log("No running MusicVault.exe (or could not stop)")
...
def _install_uninstall_helper(install_dir: Path) -> Path:
    ...
    # When running as a frozen one-file, sys.executable is MusicVault-Setup.exe.
