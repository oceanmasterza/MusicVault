...
MusicVault stores its configuration, logs, cache, and database under a
single per-user application data directory. On Windows — the primary
target platform — this is ``%APPDATA%/MusicVault``. Other platforms fall
...
def get_app_paths(*, base_override: Path | None = None) -> AppPaths:
    ...
    return AppPaths(
        ...
        database_file=root / "musicvault.db",
        logs_dir=logs_dir,
